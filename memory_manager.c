...
{
	uint32 *address_of_page_table;
		uint32 pa_page_table;
		...
		//change this "return" according to your answer
		 ...
		 address_of_page_table=(uint32*)kmalloc(PAGE_SIZE);



		 //memset((uint32 *)address_of_page_table,0,PAGE_SIZE);
		 for(int i=0;i<1024;i++)
		 {
			 address_of_page_table[i]=0;

		 }

	     pa_page_table=kheap_physical_address((uint32)address_of_page_table);


	     ptr_page_directory[PDX(virtual_address)]=CONSTRUCT_ENTRY(pa_page_table , PERM_PRESENT|PERM_WRITEABLE|PERM_USER);
	     tlbflush();
		...
		return (void*)address_of_page_table;
}
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for(int i=0;i<num_of_reqpages;i++)
	{
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 va= ROUNDDOWN(virtual_address , PAGE_SIZE);
		...
		struct Frame_Info* frame_inf=NULL;
		uint32 permissions;
		for(int i=0;i<size;i++)
		{
			//cprintf("va t freed = %x\n", va);
			pf_remove_env_page(e, va);
			frame_inf=get_frame_info(e->env_page_directory,(void*)va,&page_tab);
			permissions=pt_get_page_permissions(e,va);
			if((permissions&PERM_BUFFERED)==PERM_BUFFERED)
			{

				frame_inf->isBuffered=0;
				frame_inf->environment=NULL;
				//is buffered
				if((permissions&PERM_MODIFIED)==PERM_MODIFIED)
				{
					//is modified
					bufferlist_remove_page (&modified_frame_list, frame_inf);
					free_frame(frame_inf);
				}
				else
				{
					//is nt modified
					LIST_REMOVE(&free_frame_list, frame_inf);
					free_frame(frame_inf);
				}
				//pt_set_page_permissions(e,va,0,PERM_MODIFIED|PERM_BUFFERED);//27tyati
				pt_clear_page_table_entry(e, va);
				//page_tab[PTX(va)] = 0 ;
			}

			else if((permissions&PERM_PRESENT)==PERM_PRESENT)
			{

				//in working set
				unmap_frame(e->env_page_directory,(void*)va);
				//pt_set_page_permissions(e,va,0,PERM_PRESENT);//27tyati

				env_page_ws_invalidate(e,va);
			}


			get_page_table(e->env_page_directory,(void*)va,&page_tab);
			int will_remove=1;
			if(page_tab!=NULL)
			{
				for(int i=0;i<1024;i++)
...
void moveMem(struct Env* e, uint32 src_virtual_address, uint32 dst_virtual_address, uint32 size)
{
	...
	//size is num of pages

	uint32 * dst=(void*)dst_virtual_address;
	uint32 * src=(void*)src_virtual_address;
