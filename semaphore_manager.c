...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	struct Semaphore * allocated_semaphore=NULL; //pointer to semaphore
	int semaphore_id= get_semaphore_object_ID(ownerEnvID, semaphoreName);
	if(semaphore_id !=  E_SEMAPHORE_NOT_EXISTS)
	{
		//semaphore is already exists
		return E_SEMAPHORE_EXISTS;
	}
	else
	{
		int success_semaphoreID=allocate_semaphore_object(&allocated_semaphore);

		if(success_semaphoreID==E_NO_SEMAPHORE)
		{
			//semaphore array is full
			return E_NO_SEMAPHORE;
		...
		else
		{
			...
			return success_semaphoreID;

		}
	}
	//change this "return" according to your answer
	cprintf("in create semaphore");
	return 0;
}
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	{
		semaphores[semaphore_id].value--;
		int semaphore_value=semaphores[semaphore_id].value;

		if(semaphore_value<0)
		{
			//negative
			sched_remove_ready(myenv);
			enqueue(&(semaphores[semaphore_id].env_queue),myenv );
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	if(semaphore_id == E_SEMAPHORE_NOT_EXISTS)
		...
		else
		{
			...
			if(semaphore_value<=0)
			{
				struct Env* env=dequeue(&(semaphores[semaphore_id].env_queue));
