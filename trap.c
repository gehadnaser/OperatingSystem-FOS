...
void modified_clock_try1(struct Env * curenv,uint32 ws_max,struct WorkingSetElement* ptr_start_ws)
{
	uint32 last_index=curenv->page_last_WS_index%ws_max;
	uint32 va;
	uint32 permissions;

	for(int count=0;count<ws_max;count++)
	{
		va=ptr_start_ws[last_index].virtual_address;
		permissions=pt_get_page_permissions(curenv,va);

		if( ((permissions & PERM_MODIFIED)!=PERM_MODIFIED ) && ((permissions & PERM_USED)!=PERM_USED))
		{

			victim_find=1;
			curenv->page_last_WS_index=last_index;
			return;
		}

		last_index++;
		last_index=last_index%ws_max;
...
void modified_clock_try2(struct Env * curenv,uint32 ws_max,struct WorkingSetElement* ptr_start_ws)
{
	uint32 last_index=curenv->page_last_WS_index%ws_max;
	uint32 va;
	uint32 permissions;
	...
	for(int count=0;count<ws_max;count++)
	{
		...
		else if ( ((permissions & PERM_USED)==PERM_USED))
		{
...
void modified_clock_1(struct Env * curenv)
{
	victim_find=0;
	uint32 ws_max=curenv->page_WS_max_size;
	struct WorkingSetElement* ws_start=curenv->ptr_pageWorkingSet;
	while(victim_find==0)
	{

		modified_clock_try1(curenv,ws_max,ws_start);

		if(victim_find==0)
		{
			modified_clock_try2(curenv,ws_max,ws_start);
...
void algo(struct Env * curenv)
{
	if (isPageReplacmentAlgorithmModifiedCLOCK()){

		modified_clock_1(curenv);
...
void update_victim_info(struct Env * curenv,uint32 victim_va)
{
	uint32* ptr_pt=NULL;
	struct Frame_Info* frame_info=get_frame_info(curenv->env_page_directory,(void*)victim_va,&ptr_pt);
	...
	else
	{
		...
		if(size==max_len)
		{
			...
			LIST_FOREACH(frame_info, &modified_frame_list)
			{
				int rett=pf_update_env_page(frame_info->environment,(void*)frame_info->va,frame_info);
				pt_set_page_permissions(frame_info->environment,frame_info->va,0,PERM_MODIFIED);
				bufferList_add_page(&free_frame_list,frame_info);
	...
	env_page_ws_invalidate(curenv,victim_va);
...
void placement(struct Env * curenv, uint32 fault_va)
{
	...
	if((permissions&PERM_BUFFERED)==PERM_BUFFERED)
	{
		...
		frame_info=get_frame_info(curenv->env_page_directory,(void*)fault_va,&ptr_pt);
	...
	else
	{
		...
		int allocate_ret=allocate_frame(&frame_info);
		if(allocate_ret==0)
		{
			int map_ret=map_frame(curenv->env_page_directory,frame_info,(void*)fault_va,PERM_PRESENT|PERM_USER|PERM_WRITEABLE);
			if(map_ret==0)
			{
				int read_ret = pf_read_env_page(curenv,(void*) fault_va);
				if (read_ret == E_PAGE_NOT_EXIST_IN_PF)
				{
					if(fault_va>=USTACKBOTTOM&&fault_va<USTACKTOP)
					{
						...
						panic("not in PF & not stack page");
				}
	...
	for( int i =0 ; i<size;i++)
	{
		if(env_page_ws_is_entry_empty(curenv,curenv->page_last_WS_index)==1)
			break;
		else if(env_page_ws_is_entry_empty(curenv,i)==1)
		{
...
void __page_fault_handler_with_buffering(struct Env * curenv, uint32 fault_va)
{
	...
	{
		//find victim element in workingset and clear it
		algo(curenv);
		uint32 victim_va=curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address;
		update_victim_info(curenv,victim_va);
