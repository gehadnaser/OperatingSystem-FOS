...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	//TODO: [PROJECT 2019 - MS2 - [6] Shared Variables: Creation] createSharedObject() [Kernel Side]
		...
		int check_allocate=allocate_share_object(&share_obj);
		if(check_allocate ==E_NO_SHARE)
		{
			return E_NO_SHARE;
		}
		...
		for(int i=0;i< num_of_reqpages;i++)
		{
			...
			int check_allocate_frame=allocate_frame(&frame_inf);
			if(check_allocate_frame!=E_NO_MEM)
			{
				if(isWritable!=1)
				{
				map_frame(page_directory,frame_inf,(virtual_address+(i*PAGE_SIZE)),PERM_PRESENT|PERM_USER|PERM_WRITEABLE);
				}
				else
				{
			     map_frame(page_directory,frame_inf,(virtual_address+(i*PAGE_SIZE)),(PERM_PRESENT|PERM_USER|PERM_WRITEABLE)|PERM_WRITEABLE);
				}
				add_frame_to_storage(share_obj->framesStorage,frame_inf,i);
			}

		}
		share_obj->ownerID=ownerID;
		//share_obj->name=shareName;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	//TODO: [PROJECT 2019 - MS2 - [6] Shared Variables: Get] getSharedObject() [Kernel Side]
		...
		struct Share * shar_obj=&shares[id];
		int rounded_size=ROUNDUP(shar_obj->size,PAGE_SIZE);
	    int num_of_reqpages=rounded_size/PAGE_SIZE;
	    for(int i=0;i<num_of_reqpages;i++)
	    {
		struct Frame_Info* frame=get_frame_from_storage(shar_obj->framesStorage,i);
		if(shar_obj->isWritable==1)
		{

		   map_frame(page_directory,frame,(virtual_address+(i*PAGE_SIZE)),PERM_PRESENT|PERM_USER|PERM_WRITEABLE);
		}
		else
		{
			map_frame(page_directory,frame,(virtual_address+(i*PAGE_SIZE)),PERM_PRESENT|PERM_USER);
		}
	    }
...
int freeSharedObject(int32 sharedObjectID, void *startVA)
{
	struct Env* myenv = curenv; //The calling environment
	   ...
	   if (sharedObjectID >= MAX_SHARES)
	   {
	    ...
	    for(int i=0;i<num_of_reqpages;i++)
	    {
	    	unmap_frame(page_directory,(startVA+(i*PAGE_SIZE)));
	    	uint32 va=(uint32)startVA+(i*PAGE_SIZE);
	    	env_page_ws_invalidate(myenv,(uint32)va);
	    	uint32 *page_table=NULL;
	    	int is_empty=1;
	    	get_page_table(page_directory,(startVA+(i*PAGE_SIZE)),&page_table);
	    	if(page_table!=NULL)
	    	{
