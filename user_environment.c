...
void __env_free_with_buffering(struct Env *e)
{
	...
	{
		frame_inf=get_frame_info(e->env_page_directory,(void*)i,&page_tab);
		permissions=pt_get_page_permissions(e,i);
		if((permissions&PERM_BUFFERED)==PERM_BUFFERED)
		{
			...
			{
				//is nt modified
				bufferlist_remove_page (&free_frame_list, frame_inf);
				free_frame(frame_inf);
			}

			pt_set_page_permissions(e,i,0,PERM_MODIFIED|PERM_BUFFERED);
		}
		...
		else if((permissions&PERM_PRESENT)==PERM_PRESENT)
		{
			...
			pt_set_page_permissions(e,i,0,PERM_PRESENT);//27tyati

			env_page_ws_invalidate(e,i);
		}
		//2 working set
		get_page_table(e->env_page_directory,e->ptr_pageWorkingSet,&page_tab);
		if(e->page_WS_max_size>PAGE_SIZE)
		{
			panic("gehad w amira free env ");
		}
		uint32 ws_currentsize=env_page_ws_get_size(e);
		if(ws_currentsize==0)
		{
			kfree(page_tab);
		}
		...
		//5
		get_page_table(e->env_page_directory,(void*)i,&page_tab);
		int pt_will_remove=1;
		...
		if(page_tab!=NULL)
		{
			for(int i=0;i<1024;i++)
			{
				...
				if((page_tab[i]&PERM_PRESENT)==PERM_PRESENT)
				{
	...
	//6
	get_page_table(e->env_page_directory,(void*)e->env_page_directory,&page_tab);
	kfree(page_tab);
