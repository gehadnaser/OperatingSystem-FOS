...
struct Env* fos_scheduler_MLFQ()
{
	...
	//cprintf("current = %s @ level %d\n", curenv == NULL? "NULL" : curenv->prog_name, MLFQCurLevel-1);
	if (curenv != NULL) {
		if (MLFQCurLevel < num_of_ready_queues) {
			enqueue(&(env_ready_queues[MLFQCurLevel]), curenv);
			MLFQCurLevel += 1;
		} else {
			enqueue(&(env_ready_queues[MLFQCurLevel - 1]), curenv);
		}
	}
	...
	if (found) {
		kclock_set_quantum(quantums[MLFQCurLevel - 1]);

		//cprintf("MLFQ quantum comparison: %d - %d\n", kclock_read_cnt0_latch() , TIMER_DIV((1000/quantums[MLFQCurLevel - 1])));
		next_env = dequeue(&(env_ready_queues[MLFQCurLevel - 1]));
	}
	//cprintf("next = %s from level = %d\n", next_env == NULL? "NULL" : next_env->prog_name, MLFQCurLevel-1);
